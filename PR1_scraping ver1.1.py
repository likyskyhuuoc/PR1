...
#For some unknown reason, we cannot acquire the robots.txt information from the governmental website of Canary Islands.
#However, this function works without problem for other websites.
#print(get_robots_text('http://www.gobiernodecanarias.org'))
...
#In this code, we use headless Chrome to realise the web scraping, and the head information is exactly the same as if it were a real Chrome browser. As a result, we are not going to change the head information.
#However, for demonstration purpose, we show the codes in hashtag.

# Enter the configuration of headless browser
#options = webdriver.ChromeOptions()
# Configure the language as simplified Chinese
